...
def get_bundles(insam, ignore_umi=False, subset=None, quality_threshold=0,
                ...
                detection_method="MAPQ", dist_threshold=1000):
    ...
    last_pos = 0
    bundle_pos = -1 # how I will group reads that are within dist_threshold of each other into one bundle
    prev_bundle_pos = -1 # how I will group reads that are within dist_threshold of each other into one bundle
    last_chr = ""
    reads_dict = collections.defaultdict(
        lambda: collections.defaultdict(
            lambda: collections.defaultdict(dict))) # I think this is initialization
    read_counts = collections.defaultdict(
        lambda: collections.defaultdict(dict)) # I think this in initialization

    if chrom:
        inreads = insam.fetch(reference=chrom) # read in just reads for a chrom
    else:
        inreads = insam.fetch(until_eof=True) # read in all reads

    for read in inreads:
        if ignore_umi:
            umi = ""
        else:
            umi = read.qname.split(":")[-1] #Checked & Fixed

#        sys.stderr.write("Read: reverse %i, pos %i, aend %i, chr %s, umi %s\n" % (read.is_reverse, read.pos, read.aend, read.tid, umi))
        if (read.tid != last_chr): #Reset bundle coordinates for each chromosome.
            bundle_pos = -1
            prev_bundle_pos = -1

        if subset:
            if random.random() >= subset: # Randomly sample subset% of reads
                continue

        if quality_threshold:
            if read.mapq < quality_threshold: # Remove low quality mappings
                continue

        if read.is_unmapped: # Remove unmapped reads
            continue

        if read.mate_is_unmapped and paired: # Remove paired reads where not both ends mapped
            continue

        if read.is_read2: # Remove mate of paired reads
            continue
...
#                    pos = pos - read.cigar[0][1]
            ...
            if whole_contig:
                ...
                do_output = start > (last_pos+dist_threshold) or not read.tid == last_chr # this is somewhat wrong the 1000 that used to be here is not a distance threshold but a "chunk" size I believe, that is there to optimize memory performance

            if do_output:

                out_keys = [x for x in reads_dict.keys() if x <= start-dist_threshold] # this is somewhat wrong the 1000 that used to be here is not a distance threshold but a "chunk" size I believe, that is there to optimize memory performance
            ...
            # group reads mapping between bundle_pos and bundle_pos+dist_threshold to bundle_pos
            # If dist_threshold = 0 then this is exactly the same as the unmodified script

            if (bundle_pos < 0):
                bundle_pos = pos
		prev_bundle_pos = pos
            elif ((pos >= bundle_pos) and (pos <= bundle_pos+dist_threshold)):
                pos = bundle_pos
            elif (pos > bundle_pos+dist_threshold):
                prev_bundle_pos = bundle_pos
                bundle_pos = pos
            elif (pos < bundle_pos): #Can happen due to reverse reads
                if ((pos >= prev_bundle_pos) and (pos <= prev_bundle_pos+dist_threshold)): # Does it fit in the previous bundle?
                     pos = prev_bundle_pos
		     sys.stderr.write("Mapped to prev bundle\n")
                elif (pos < prev_bundle_pos):
                    sys.stderr.write("Warning: Reversed read mapping to %i doesn't fit in current, %i, or previous bundle, %i, thus keeps its mapping position\n" % (pos,bundle_pos,prev_bundle_pos))
#                else:
#        sys.stderr.write("bundle pos  %i, prev bundle pos %i, pos %i\n" % (bundle_pos,prev_bundle_pos,pos))
...
def main(argv=None):
    ...
    options_in_sam = False
    options_out_sam = False
    options_ignore_umi=False
    options_subset = 1.1
    options_spliced = False
    options_soft = 4
    options_bundle_threshold = int(sys.argv[2] )#distance threshold for bundles = Second cmd line arg
    options_threshold = 1 #distance threshold for UMI differences (n-mismatches)
#    sys.stderr.write("Distance Threshold %i\n" % options_bundle_threshold)
    options_chrom = None
    options_paired = False
#    options_method = "directional-adjacency"
#    options_method = "adjacency"
#    options_method = "unique"
    options_method = sys.argv[4] #method = fourth cmd line arg
    options_stats = False
    options_further_stats = False
    options_per_contig = True
    options_whole_contig = True
    options_detection_method = "NH"
    options_mapping_quality = 0
    options_read_length = False 
    options_stdin_name = sys.argv[1] #input bam name = First cmd line arg
    options_stdout_name = sys.argv[3] #output bam name? = Third cmd line arg
    ...
    in_name = options_stdin_name
    out_name = options_stdout_name
    ...
    if options_detection_method:
        bam_features = detect_bam_features(infile.filename)

        if not bam_features[options_detection_method]:
    ...
    if options_stats:
        ...
        read_gn = random_read_generator(infile.filename, chrom=options_chrom)

    for bundle in get_bundles(infile,
                              ignore_umi=options_ignore_umi,
                              subset=float(options_subset),
                              quality_threshold=options_mapping_quality,
                              paired=options_paired,
                              chrom=options_chrom,
                              spliced=options_spliced,
                              soft_clip_threshold=options_soft,
                              per_contig=options_per_contig,
                              whole_contig=options_whole_contig,
                              read_length=options_read_length,
                              detection_method=options_detection_method,
                              dist_threshold=options_bundle_threshold):

        nInput += sum([bundle[umi]["count"] for umi in bundle])

        if nOutput % 10000 == 0:
            sys.stderr.write("Outputted %i\n" % nOutput)

        if nInput % 1000000 == 0:
            sys.stderr.write("Read %i input reads\n" % nInput)
        ...
        if options_ignore_umi:
            ...
            processor = ClusterAndReducer(options_method)

            # dedup using umis and write out deduped bam

            reads, umis, umi_counts, topologies, nodes = processor(
                bundle, options_threshold,
                options_stats, options_further_stats)
    ...
    if options_stats:
        ...
        UMI_counts_df.to_csv(
            options_stats + "_per_umi_per_position.tsv", sep="\t")
        ...
        agg_df.to_csv(options_stats + "_per_umi.tsv", sep="\t")
        ...
        edit_distance_df = pd.DataFrame({
            ...
            options_method: tallyCounts(post_cluster_binned, max_ed),
            "%s_null" % options_method: tallyCounts(post_cluster_null_binned, max_ed),
            "edit_distance": cluster_bins})

        # TS - set lowest bin (-1) to "Single_UMI"
        edit_distance_df['edit_distance'][0] = "Single_UMI"

        edit_distance_df.to_csv(options_stats + "_edit_distance.tsv",
                                index=False, sep="\t")

        if options_further_stats:
            with IOTools.openFile(options_stats + "_topologies.tsv", "w") as outf:
                outf.write(
                    "\n".join(["\t".join((x, str(y)))
                               for x, y in topology_counts.most_common()]) + "\n")

            with IOTools.openFile(options_stats + "_nodes.tsv", "w") as outf:
                outf.write(
                    "\n".join(["\t".join(map(str, (x, y))) for
                               x, y in node_counts.most_common()]) + "\n")

    # write footer and output benchmark information.
    sys.stderr.write("Input file: %s Method: %s \n" % (options_stdin_name, options_method))
    sys.stderr.write("Number of reads in: %i, Number of reads out: %i\n" %
           (nInput, nOutput))
    #E.Stop()
