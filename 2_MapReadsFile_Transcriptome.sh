...
if [[ $FILE1TOMAP =~ \.gz$ ]] ; then
    ...
    $STAR --runThreadN $NUMTHREADS --runMode alignReads --quantMode TranscriptomeSAM --readFilesIn $FILE1TOMAP $FILE2TOMAP --readFilesCommand zcat --parametersFiles $PARAMFILE --outFileNamePrefix $OUTDIR/$FILEnopath --outTmpDir $WORKINGDIR
else
    FILEnopath=`basename ${FILE1TOMAP%.fq}`
    $STAR --runThreadN $NUMTHREADS --runMode alignReads --quantMode TranscriptomeSAM --readFilesIn $FILE1TOMAP $FILE2TOMAP --parametersFiles $PARAMFILE --outFileNamePrefix $OUTDIR/$FILEnopath --outTmpDir $WORKINGDIR
fi
